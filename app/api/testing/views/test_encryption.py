...
from api.encryption import EncryptionAES
...
class TestEncryption(TestCase):
    ...
    def test_encryption_correct(self):
        encrypted = self.encryption_aes.encrypt(self.test_data, self.test_key, self.test_iv)
        decrypted = self.encryption_aes.decrypt(encrypted, self.test_key, self.test_iv)
        self.assertEqual(decrypted, self.test_data)

    def test_encrypt_with_wrong_key(self):
        encrypted = self.encryption_aes.encrypt(self.test_data, self.test_key, self.test_iv)
        new_key = self.encryption_aes.create_key()
        decrypted = self.encryption_aes.decrypt(encrypted, new_key, self.test_iv)
